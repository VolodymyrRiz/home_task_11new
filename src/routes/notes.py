...
from src.schemas import NoteBase, NoteResponse
...
async def create_note(body: NoteBase, db: Session = Depends(get_db)):
    return await repository_notes.create_note(body, db)


@router.put("/{note_id}", response_model=NoteResponse)
async def update_note(body: NoteBase, note_id: int, db: Session = Depends(get_db)):
...
async def update_status_note(body: NoteBase, note_id: int, db: Session = Depends(get_db)):
